...
<body>
    ...
    <div class="footer">
        Generated on {{ date('d M Y, H:i') }} &bull; Page header for identification
    </div>

    <div class="doc-title-block">
        <div class="doc-title">Staff Registry Directory</div>
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            <tr>
                <td style="font-weight: bold;">{{ $item->staff_number }}</td>
                <td>{{ $item->first_name }} {{ $item->last_name }}</td>
                <td>{{ $item->department?->name ?? '—' }}</td>
                <td>
                    @php 
                        $roles = $item->user ? $item->user->getRoleNames()->join(', ') : 'Teacher';
                    @endphp
                    {{ $roles }}
                </td>
                <td>{{ $item->email }}</td>
