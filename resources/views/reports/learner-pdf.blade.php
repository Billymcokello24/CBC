...
<body>
    ...
    <div class="doc-title-block">
        <div class="doc-title">Student Directory Registry</div>
        <div style="font-size: 9px; color: #64748b; margin-top: 5px;">
            Institutional Baseline Records | Total Records: {{ $learners->count() }}
        </div>
    </div>
    ...
    <table class="data-table">
        <thead>
            <tr>
                <th width="35%">Learner Full Name</th>
        ...
        <tbody>
            @foreach($learners as $l)
                <tr>
                    ...
                    <td align="right"><span class="badge {{ $l->status === 'active' ? 'status-active' : 'status-inactive' }}">{{ ucfirst($l->status) }}</span></td>
                </tr>
            @endforeach
            @if($learners->isEmpty())
                <tr>
                    <td colspan="5" align="center" style="padding: 20px;">No students found in registry.</td>
                </tr>
            @endif
        </tbody>
    </table>
