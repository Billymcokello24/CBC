...
const selectedArea = ref(props.filters.learning_area_id || 'all');
...
const applyFilters = (pageNumber?: number) => {
    router.get(
        '/curriculum/subjects',
        {
            ...
            learning_area_id: selectedArea.value,
...
watch([selectedStatus, selectedArea, perPage], () => applyFilters());

const filteredSubjects = computed(() => {
    // Note: Filtering is handled server-side now, but we keep this for UI if needed
    return props.subjects.data;
});
...
const confirmDeleteSubject = (id: number) => {
    if (window.confirm('Are you sure you want to delete this subject? This cannot be undone.')) {
        router.delete(`/curriculum/subjects/${id}`);
    }
};
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        >
            ...
            <div class="flex flex-col gap-6 px-1 md:flex-row md:items-center md:justify-between">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <Button
                        ...
                        class="h-10 rounded-lg border-border bg-card px-4 text-xs font-semibold hover:bg-muted"
                    >
                        <a :href="`/curriculum/subjects/export-pdf?search=${searchQuery}&status=${selectedStatus}&learning_area_id=${selectedArea}`" target="_blank">
            ...
            <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                <div
                    v-for="(stat, idx) in [
                        { label: 'Total Subjects', val: stats.total, sub: 'Across all areas', icon: BookOpen },
                        { label: 'Learning Areas', val: learningAreas.length, sub: 'Subject groups', icon: GraduationCap },
                        { label: 'Active Subjects', val: stats.active, sub: 'Currently taught', icon: CheckCircle2 },
            ...
            <div class="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
                ...
                <div class="p-6">
                    <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                        ...
                        <div class="space-y-2">
                            <Label class="text-[10px] font-bold text-muted-foreground uppercase opacity-40 ml-1">Operational Status</Label>
                            <div class="relative">
                                <select v-model="selectedStatus" class="h-11 w-full cursor-pointer appearance-none rounded-xl border border-border bg-muted/20 px-4 text-xs font-bold outline-none focus:bg-background focus:ring-2 focus:ring-primary/20 transition-all uppercase tracking-tighter">
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-border/50">
                            ...
                            >
                                ...
                                <td class="px-6 py-4">
                                    <Badge :class="subject.is_active ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950/30 dark:text-emerald-400' : 'bg-slate-50 text-slate-400'" class="rounded-lg border-0 px-2.5 py-1 text-[9px] font-bold uppercase tracking-widest">
