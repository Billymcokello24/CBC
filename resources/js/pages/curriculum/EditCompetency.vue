...
import { ArrowLeft, Loader2, Save, BrainCircuit, ShieldCheck } from 'lucide-vue-next';
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Modify Pillar', href: route('curriculum.competencies.edit', props.competency.id) }
];
...
const form = useForm({ 
    ...
    display_order: props.competency.display_order, 
    is_active: props.competency.is_active 
});
...
})).put(route('curriculum.competencies.update', props.competency.id));
</script>

<template>
    <Head title="Modify Core Competency" />
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-8 p-8 font-pulsar mt-2 max-w-[1200px] mx-auto animate-in fade-in duration-500">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold tracking-tight">Modify Core Pillar</h1>
                    <p class="text-muted-foreground font-medium uppercase text-[10px] tracking-widest">{{ competency.name }} • PILLAR_NODE_{{ competency.id }}</p>
                </div>
            </div>
                    ...
                    <form @submit.prevent="submit" class="space-y-8 rounded-[2.5rem] border bg-card p-10 shadow-sm border-t-8 border-t-violet-500">
                        <div class="grid gap-8">
                            <div class="grid gap-4 sm:grid-cols-2 text-left">
                                <div class="space-y-2">
                                    <Label for="name" class="text-[10px] font-black uppercase tracking-widest text-slate-400 pl-1">Pillar Nomenclature</Label>
                                    <Input id="name" v-model="form.name" class="h-12 border-slate-200 rounded-xl focus:ring-violet-500" placeholder="e.g. Critical Thinking" />
                                    <InputError :message="form.errors.name" />
                                </div>
                                <div class="space-y-2">
                                    <Label for="code" class="text-[10px] font-black uppercase tracking-widest text-slate-400 pl-1">Registry Code</Label>
                                    <Input id="code" v-model="form.code" class="h-12 border-slate-200 rounded-xl uppercase font-bold focus:ring-violet-500" placeholder="e.g. CT-PILLAR" />
                            ...
                            <div class="grid gap-4 sm:grid-cols-2 text-left">
                                <div class="space-y-2">
                                    <Label for="category" class="text-[10px] font-black uppercase tracking-widest text-slate-400 pl-1">Behavioral Category</Label>
                                    <Input id="category" v-model="form.category" class="h-12 border-slate-200 rounded-xl focus:ring-violet-500" placeholder="e.g. Cognitive" />
                                    <InputError :message="form.errors.category" />
                                </div>
                                <div class="space-y-2">
                                    <Label for="display_order" class="text-[10px] font-black uppercase tracking-widest text-slate-400 pl-1">Sequence Order</Label>
                            ...
                            <div class="space-y-2 text-left">
                                <Label for="description" class="text-[10px] font-black uppercase tracking-widest text-slate-400 pl-1">Pillar Specification</Label>
                                <Textarea id="description" v-model="form.description" class="min-h-[120px] border-slate-200 rounded-2xl focus:ring-violet-500 p-4" placeholder="Specify the behavioral indicators and scope of this core competency..." />
                            ...
                            <div class="flex items-center gap-3 p-4 rounded-2xl bg-slate-50 border shadow-inner max-w-xs transition-all hover:bg-slate-100">
                                ...
                                <Label for="is_active" class="text-xs font-black uppercase tracking-widest cursor-pointer" :class="form.is_active ? 'text-emerald-600' : 'text-slate-400'">
                                    {{ form.is_active ? 'Logic Pillar: Active' : 'Logic Pillar: Dormant' }}
                                </Label>
                            </div>
                        </div>
                        ...
                        <div class="pt-8 flex items-center justify-end gap-3 border-t border-slate-50">
                            <Button type="button" variant="outline" class="h-12 px-8 rounded-2xl font-black text-xs uppercase tracking-widest" as-child>
                                <Link :href="route('curriculum.competencies')">Discard Changes</Link>
                            </Button>
                     ...
                     <div class="rounded-[2.5rem] bg-slate-950 p-10 shadow-2xl text-white relative overflow-hidden group border-[12px] border-slate-900">
                          ...
                          <div class="relative z-10 space-y-6">
                               <h3 class="text-xl font-black uppercase tracking-tighter flex items-center gap-3">
                                   <ShieldCheck class="h-6 w-6 text-violet-400" /> Core Validation
                               </h3>
                               <p class="text-sm font-medium text-slate-400 leading-relaxed italic opacity-80">
                                   Core competencies represent the behavioral anchors of the CBC framework. Modifying these impacts all related performance indicators.
                               </p>
                               <div class="pt-6 border-t border-white/10 space-y-4">
                                    ...
                                    <div class="flex items-center justify-between text-[10px] font-black uppercase tracking-widest">
                                         ...
                                         <span class="text-violet-400 uppercase italic">System Core Pillar</span>
                                    </div>
                               </div>
                          </div>
                     </div>

                     <div class="rounded-[2rem] border bg-slate-50 p-8 shadow-inner space-y-4 text-left">
                          <h4 class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Global Identifiers</h4>
                          <div class="space-y-2">
                               <div class="text-xs font-bold text-slate-900">P_NODE_ID: {{ competency.id }}</div>
                               <div class="text-xs font-bold text-slate-900 uppercase">UID: PILLAR_{{ competency.code }}_2026</div>
                          </div>
                     </div>
                </div>
            </div>
