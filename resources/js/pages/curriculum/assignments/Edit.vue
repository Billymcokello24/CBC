...
import {
    ...
    BookOpen, Target, Layers, Download
...
const form = useForm({
    ...
    assignment_type: props.assignment.assignment_type || 'homework',
    due_date: props.assignment.due_date ? new Date(props.assignment.due_date).toISOString().split('T')[0] : '',
    assigned_date: props.assignment.assigned_date ? new Date(props.assignment.assigned_date).toISOString().split('T')[0] : '',
    total_marks: props.assignment.total_marks,
    class_id: props.assignment.class_id,
    subject_id: props.assignment.subject_id,
    strand_id: props.assignment.strand_id || '',
    sub_strand_id: props.assignment.sub_strand_id || '',
    academic_term_id: props.assignment.academic_term_id || 1,
    attachments: [] as File[],
});
...
watch(() => form.strand_id, (newVal, oldVal) => {
    if (oldVal !== undefined) {
        form.sub_strand_id = '';
    }
});
...
const deleteExistingAttachment = (id: number) => {
    if (confirm('Permanently delete this file?')) {
        router.delete(route('curriculum.assignments.attachments.destroy', id), {
            preserveScroll: true,
        });
    }
};
...
// Submit
const submit = () => {
    form.post(route('curriculum.assignments.update', props.assignment.id), {
        forceFormData: true,
    });
};
</script>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-6 font-sans max-w-[1200px] mx-auto bg-[#f9fafb]/30">
            ...
            <div class="flex items-center justify-between gap-4">
                <div class="space-y-1">
                    <Link :href="`/curriculum/assignments/${assignment.id}`" class="group inline-flex items-center gap-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-wider hover:text-blue-600 transition-all mb-1">
                        <ArrowLeft class="h-3 w-3 group-hover:-translate-x-1 transition-transform" /> Back to Assignment
                    </Link>
                    <h1 class="text-2xl font-bold tracking-tight text-slate-900">Edit Assignment</h1>
                </div>
                <div class="flex items-center gap-3">
                    ...
                    <Button @click="submit" :disabled="form.processing" class="h-10 px-6 rounded-xl bg-blue-600 hover:bg-blue-700 font-semibold text-xs text-white shadow-sm transition-all">
                        <Save class="mr-2 h-4 w-4" /> {{ form.processing ? 'Saving...' : 'Update Assignment' }}
                    </Button>
                </div>
            </div>
            ...
            <form @submit.prevent="submit" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 flex flex-col gap-6">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
                        ...
                        <!-- Existing Files -->
                        <div v-if="assignment.attachments?.length" class="space-y-2">
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1 mb-2">Current Files</p>
                            <div v-for="file in assignment.attachments" :key="file.id" class="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-100 group">
                                <div class="flex items-center gap-3">
                                    <span class="text-lg">{{ getFileIcon(file.file_type) }}</span>
                                    <div class="flex flex-col">
                                        <span class="text-xs font-bold text-slate-700">{{ file.file_name }}</span>
                                        <span class="text-[10px] font-medium text-slate-400">{{ formatFileSize(file.file_size) }}</span>
                                    </div>
                                </div>
                                <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                                    <a :href="`/curriculum/assignments/attachments/${file.id}/download`" class="h-7 w-7 rounded-lg flex items-center justify-center text-slate-400 hover:bg-blue-50 hover:text-blue-600 transition-all">
                                        <Download class="h-3.5 w-3.5" />
                                    </a>
                                    <button type="button" @click="deleteExistingAttachment(file.id)" class="h-7 w-7 rounded-lg flex items-center justify-center text-slate-400 hover:bg-red-50 hover:text-red-500 transition-all">
                        ...
                        <div
                            @drop.prevent="isDragOver = false; addFiles($event.dataTransfer?.files || null)"
                        ...
                        >
                            ...
                            <div>
                                <p class="text-sm font-semibold text-slate-600">Add more files</p>
                                <p class="text-[10px] font-medium text-slate-400 mt-0.5">Drop here or <span class="text-blue-600">browse</span></p>
                            </div>
                        </div>
                        <input ref="fileInput" type="file" multiple class="hidden" @change="handleFileSelect" />

                        <!-- New File List -->
                        <div v-if="form.attachments.length" class="space-y-2 pt-2">
                             <p class="text-[10px] font-bold text-blue-400 uppercase tracking-widest pl-1 mb-2 italic">Newly Added</p>
                            <div v-for="(file, index) in form.attachments" :key="index" class="flex items-center justify-between p-3 rounded-xl bg-blue-50/30 border border-blue-100 group">
                                <div class="flex items-center gap-3">
                                    <span class="text-lg">{{ getFileIcon(file.type) }}</span>
                                    <div class="flex flex-col">
                                        <span class="text-xs font-bold text-blue-800">{{ file.name }}</span>
                                        <span class="text-[10px] font-medium text-blue-400">{{ formatFileSize(file.size) }}</span>
                                    </div>
                                </div>
                                <button type="button" @click="removeNewFile(index)" class="h-7 w-7 rounded-lg flex items-center justify-center text-blue-300 hover:bg-red-50 hover:text-red-500 transition-all">
                                    <X class="h-3.5 w-3.5" />
                                </button>
                            </div>
                        </div>
                ...
                <div class="flex flex-col gap-6">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-3">
                        <Button @click="submit" :disabled="form.processing" class="w-full h-11 rounded-xl bg-blue-600 hover:bg-blue-700 font-bold text-xs text-white shadow-sm uppercase tracking-wider transition-all">
                            <Save class="mr-2 h-4 w-4" /> {{ form.processing ? 'Updating...' : 'Save Changes' }}
                        </Button>
                        <Link :href="`/curriculum/assignments/${assignment.id}`">
