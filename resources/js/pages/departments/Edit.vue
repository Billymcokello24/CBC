...
const name = ref(dept.name);
const code = ref(dept.code);
const description = ref(dept.description);
const head = ref(dept.head_of_department_id || '');
const isActive = ref(dept.is_active);

const submit = () => {
  router.put(`/departments/${dept.id}`, { name: name.value, code: code.value, description: description.value, head_of_department_id: head.value, is_active: isActive.value });
};
</script>
