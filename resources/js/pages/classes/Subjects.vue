...
const openAssignModal = (subject: SubjectRow) => {
    ...
    assignmentForm.academic_year_id =
        subject.academic_year_id || props.classroom.academic_year_id;
...
const saveAssignment = () => {
    ...
    assignmentForm.put(
        `/classes/allocations/${selectedSubject.value.allocation_id}`,
        {
            preserveScroll: true,
            onSuccess: () => {
                closeAssignModal();
...
const confirmAction = () => {
    ...
    actionForm
        .transform(() => ({
            ...
            class_id: props.classroom.id,
            academic_year_id:
                selectedSubject.value?.academic_year_id ??
                props.classroom.academic_year_id,
            is_primary_teacher:
                selectedSubject.value?.is_primary_teacher ?? false,
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="mx-auto max-w-[1600px] animate-in space-y-6 p-4 pb-10 duration-700 fade-in slide-in-from-bottom-4 sm:space-y-8 sm:p-6 sm:pb-20 md:p-8">
            ...
            <div class="rounded-2xl border border-border bg-card p-6 shadow-sm dark:border-white/5">
                ...
                >
                    <div
                        ...
                        :key="subject.allocation_id"
                        class="group relative overflow-hidden rounded-xl border bg-card transition hover:border-primary/40 hover:shadow-md"
                    >
                        <Link
                            :href="`/curriculum/subjects/${subject.subject_id}`"
                            class="block h-full p-5 pr-16"
                        >
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    <h3
                                        class="text-lg font-semibold transition group-hover:text-primary"
                                    >
                                        {{ subject.name }}
                                    </h3>
                                    <p class="text-sm text-muted-foreground">
                        ...
                        <div class="absolute top-3 right-3 z-10">
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                    ...
                                    <DropdownMenuItem as-child>
                                        <Link
                                            :href="`/curriculum/subjects/${subject.subject_id}/edit`"
                                        >
                                    ...
                                    >
                                        <UserCheck class="mr-2 h-4 w-4" />Assign
                                        Teacher
                                    </DropdownMenuItem>
                                    <DropdownMenuItem as-child>
                                        <Link
                                            :href="`/curriculum/subjects/${subject.subject_id}/allocate`"
                                        >
                                            <ClipboardList
                                                class="mr-2 h-4 w-4"
                                            />Allocate to Grades
                                    ...
                                    <DropdownMenuItem
                                        ...
                                        @click="
                                            openConfirmModal('delete', subject)
                                        "
                                    >
                ...
                <div v-else class="overflow-x-auto rounded-xl border">
                    <table class="w-full">
                        <thead>
                            <tr
                                class="border-b bg-muted/50 text-left text-sm text-muted-foreground"
                            >
            ...
            >
                <div
                    class="w-full max-w-lg rounded-xl border bg-background p-6 shadow-xl"
                >
                    <h2 class="text-lg font-semibold">Assign Teacher</h2>
                    <p class="mt-1 text-sm text-muted-foreground">
                        <template v-if="selectedSubject"
                            >Update the teacher assigned to
                            {{ selectedSubject.name }} for
                            {{ classroom.name }}.</template
                        >
                    </p>
                    ...
                    <div class="mt-6 space-y-4">
                        ...
                        <div class="grid gap-4 md:grid-cols-2">
                            ...
                            >
                                <input
                                    v-model="assignmentForm.is_primary_teacher"
                                    type="checkbox"
                                />
                                Primary teacher
                            </label>
                    ...
                    <div class="mt-6 flex justify-end gap-2">
                        <Button variant="outline" @click="closeAssignModal"
                            >Cancel</Button
                        >
                        <Button
                            :disabled="
            ...
            >
                ...
                >
                    ...
                    <div class="mt-6 flex justify-end gap-2">
                        <Button variant="outline" @click="closeConfirmModal"
                            >Cancel</Button
                        >
                        <Button
                            :variant="
