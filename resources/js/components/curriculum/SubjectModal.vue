...
watch(() => props.subject, (newSubject) => {
  if (newSubject) {
    form.learning_area_id = newSubject.learning_area_id?.toString() || '';
    ...
    <DialogContent class="sm:max-w-[550px] font-pulsar">
      <DialogHeader>
        ...
        <DialogDescription class="font-medium text-slate-500 italic">
          {{ subject ? 'Synchronize subject parameters and academic metadata.' : 'Initialize a new subject node within the curriculum registry.' }}
        </DialogDescription>
      </DialogHeader>
      
      <form @submit.prevent="submit" class="grid gap-5 py-4">
        <div class="grid grid-cols-4 items-center gap-4">
          <Label class="text-right text-[10px] font-black uppercase tracking-widest text-slate-400">Learning Area</Label>
        ...
        <div class="grid grid-cols-4 items-center gap-4">
          ...
          <Input id="name" v-model="form.name" class="col-span-3 h-10 border-slate-200" placeholder="e.g. Mathematics" required />
        </div>
        
        <div class="grid grid-cols-4 items-center gap-4">
          <Label for="code" class="text-right text-[10px] font-black uppercase tracking-widest text-slate-400">Code</Label>
          <Input id="code" v-model="form.code" class="col-span-3 h-10 border-slate-200 uppercase" placeholder="e.g. MATH" required />
        </div>

        <div class="grid grid-cols-4 items-center gap-4">
          <Label class="text-right text-[10px] font-black uppercase tracking-widest text-slate-400">Subject Type</Label>
          <Select v-model="form.subject_type">
            ...
            <SelectContent class="font-pulsar">
              ...
              <SelectItem value="Applied">Applied / Technical</SelectItem>
            </SelectContent>
          </Select>
        </div>

        <div class="grid grid-cols-4 items-center gap-4">
          <Label for="description" class="text-right text-[10px] font-black uppercase tracking-widest text-slate-400">Definition</Label>
          <Textarea id="description" v-model="form.description" class="col-span-3 border-slate-200 min-h-[80px]" placeholder="Subject scope..." />
        </div>

        <div class="grid grid-cols-4 items-center gap-4">
          <Label class="text-right text-[10px] font-black uppercase tracking-widest text-slate-400">Attributes</Label>
          <div class="col-span-3 flex items-center gap-6">
            <div class="flex items-center space-x-2">
              <Checkbox id="is_examinable" :checked="form.is_examinable" @update:checked="(val: boolean) => form.is_examinable = val" />
              <Label for="is_examinable" class="text-[10px] font-bold uppercase cursor-pointer">Examinable</Label>
            </div>
            <div class="flex items-center space-x-2">
              <Checkbox id="is_active" :checked="form.is_active" @update:checked="(val: boolean) => form.is_active = val" />
              <Label for="is_active" class="text-[10px] font-bold uppercase cursor-pointer text-emerald-600">Active</Label>
            </div>
          </div>
        </div>
        ...
        <DialogFooter>
          <Button type="button" variant="ghost" @click="$emit('close')" class="font-black text-[10px] uppercase tracking-widest">Abort</Button>
