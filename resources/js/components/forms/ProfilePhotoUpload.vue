...
// Initialize preview
watch(
    () => [props.modelValue, props.currentImage],
    ([newFile, currentImg]) => {
...
<template>
    <div class="relative">
        ...
        <div v-else class="space-y-4">
            <div v-if="label" class="text-sm font-medium text-foreground">
                {{ label }}
            </div>

            <div class="flex items-center gap-6">
                <div
                    class="group relative h-24 w-24 cursor-pointer overflow-hidden rounded-2xl border-2 border-dashed border-muted-foreground/20 bg-muted/30 transition-all hover:border-indigo-600/30"
                    @click="triggerUpload"
                >
                    <img
                        v-if="previewUrl"
                    ...
                    <div
                        v-else
                        class="flex h-full w-full flex-col items-center justify-center text-muted-foreground/40"
                    >
                ...
                <div class="flex flex-col gap-2">
                    <div class="flex items-center gap-2">
                        ...
                        <Button
                            v-if="previewUrl && !internalIsUploading"
                            type="button"
                            variant="ghost"
                            size="sm"
                            @click="removePhoto"
                            class="h-9 rounded-xl text-rose-500 hover:bg-rose-50 hover:text-rose-600"
                        >
        ...
        <p v-if="error" class="mt-2 text-xs font-medium text-rose-500">
            {{ error }}
        </p>
    </div>
</template>
