...
<template>
  ...
  >
    <div v-if="show" class="fixed top-6 right-6 z-[9999] w-full max-w-sm overflow-hidden rounded-xl bg-white shadow-2xl ring-1 ring-black ring-opacity-5 pointer-events-auto">
      <div class="p-4">
        <div class="flex items-start">
          ...
          <div class="ml-3 w-0 flex-1 pt-0.5">
            <p class="text-xs font-black uppercase tracking-[0.2em]" :class="type === 'success' ? 'text-emerald-700' : type === 'error' ? 'text-rose-700' : 'text-sky-700'">
              {{ type === 'success' ? 'Task Finalized' : type === 'error' ? 'System Warning' : 'Update' }}
            </p>
            <p class="mt-1 text-sm font-bold text-gray-500 leading-relaxed uppercase tracking-tight opacity-75">
              {{ message }}
            </p>
          </div>
          <div class="ml-4 flex flex-shrink-0">
            <button 
                type="button" 
                @click="dismiss" 
                class="inline-flex rounded-md bg-white text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 transition-colors"
            >
      ...
      <!-- Progress Bar -->
      <div v-if="show" class="h-1 bg-violet-600/10 w-full overflow-hidden">
        <div class="h-full bg-violet-600 animate-[progress_5s_linear_forwards]"></div>
      </div>
    </div>
  </transition>
</template>
